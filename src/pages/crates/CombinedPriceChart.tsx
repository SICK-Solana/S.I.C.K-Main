...
const CombinedPriceChart: React.FC<CombinedPriceChartProps> = ({ tokens }) => {
  ...
  const processChartData = (priceData: PriceData[], tokens: Token[]): ChartDataPoint[] => {
    const quantityMap = new Map(tokens.map(token => [token.coingeckoId, token.quantity])); // Percentage, not actual quantity
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      priceData.forEach(coin => {
        ...
        combinedValue += (coin.sparkline_in_7d.price[i] * quantityPercentage / 100);  // Adjust for percentage
      });
